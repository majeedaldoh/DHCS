...
contract DHCs{
    ...
    function IssueCertificate(string memory _iname, string memory _pname, string memory _reason, string memory _admittedOn, string memory _dishchargedOn, address addr) public {
        Certificates[addr] = Certificate(_iname, _pname, _reason, _admittedOn, _dishchargedOn,addr);
        CertificateCount++;
        certificatesList.push(addr);
    }
    ...
    for (uint i = 0; i < certificatesList.length; i++) {
                result[i] = Certificates[certificatesList[i]];
        }
    return result;
    }
}